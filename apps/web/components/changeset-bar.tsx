...
export function ChangeSetBar({ changeSet, findings = [], onCommitted }: ChangeSetBarProps){
  ...
  async function approve(){
    ...
    setError(null);
    try {
      const response = await fetch(`/api/changesets/${changeSet.id}/approve`, {
        method:'POST',
  ...
  if(isCommitted) return <div className="changeset-bar committed">
    <div><CheckCircle2 size={16}/><b>CHANGE COMMITTED</b><small>{changeSet.id.slice(0,8)}</small><p>Immutable project version created successfully.</p></div>
  </div>;

  return <div className="changeset-bar">
    <div><span className="status-dot"/><b>PROPOSED CHANGE</b><small>{changeSet.id.slice(0,8)}</small><p>{changeSet.intentSummary}</p>{error && <em className="changeset-error">{error}</em>}</div>
