...
function harness(state='APS_WORKITEM_SUCCEEDED'){
 ...
 const reviewStore={store:async record=>{calls.push('evidence');assert.equal(record.nativeDwgReopenVerified,false);}};
 return {env,now:()=>1000,withStores:work=>work({ledger,receiptStore,reviewStore}),inspectWorkitem:async()=>{calls.push('inspect');return {state,workitemId,status:state==='APS_WORKITEM_SUCCEEDED'?'success':'inprogress',executionEnabled:false,approvalGranted:false};},finalizeArtifacts:async()=>{calls.push('artifacts');return {state:'ARTIFACTS_REQUIRE_ARCHON_REVIEW',dwgSha256:'a'.repeat(64),reportSha256:'b'.repeat(64),checklist:[],approvalGranted:false,reconciliation:'PROPOSE_CHANGESET_ONLY'};},calls};
}
...
test('success claims, decrypts, validates, stores evidence, then consumes without approval',async()=>{const h=harness();const result=await finalizeGovernedSandbox(h);assert.equal(result.approvalGranted,false);assert.deepEqual(h.calls,['lookup','inspect','claim','artifacts','evidence','consumed']);});
