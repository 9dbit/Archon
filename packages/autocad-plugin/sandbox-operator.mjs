...
export async function handleSandboxOperator(request,{env=process.env,activate,probe,artifacts,prepareInput,review,transport,issueApproval}={}) {
 ...
 try {
  ...
  if(!body||Object.keys(body).length!==1||!['ACTIVATE_LEDGER','PROBE_RESOURCES','PROBE_ARTIFACTS','PREPARE_REVIEWED_INPUT','PREPARE_SUBMISSION_REVIEW','PROBE_TRANSPORT','ISSUE_EXECUTION_APPROVAL'].includes(body.operation))return fail('SANDBOX_OPERATOR_REQUEST_INVALID',400);
  const action=body.operation==='ACTIVATE_LEDGER'?activate:body.operation==='PROBE_ARTIFACTS'?artifacts:body.operation==='PREPARE_REVIEWED_INPUT'?prepareInput:body.operation==='PREPARE_SUBMISSION_REVIEW'?review:body.operation==='PROBE_TRANSPORT'?transport:body.operation==='ISSUE_EXECUTION_APPROVAL'?issueApproval:probe;
