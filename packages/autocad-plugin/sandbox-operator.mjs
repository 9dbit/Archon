...
export async function handleSandboxOperator(request,{env=process.env,activate,probe}={}) {
 ...
 try {
  ...
  if(!body||Object.keys(body).length!==1||!['ACTIVATE_LEDGER','PROBE_RESOURCES'].includes(body.operation))return fail('SANDBOX_OPERATOR_REQUEST_INVALID',400);
  const action=body.operation==='ACTIVATE_LEDGER'?activate:probe;
