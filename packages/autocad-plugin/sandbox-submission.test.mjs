...
function config() {
 ...
 return {review,inputBytes,approval:{runId:'test-run',manifestSha256:review.manifestSha256,reference:'test-explicit-approval',expiresAt:2000},ledger,executionEnabled:true,now:()=>1000,calls,submit:async()=>{calls.push('provider');return {id:'workitem-1',privateToken:'secret'};},preflight:async()=>({manifestSha256:review.manifestSha256,submitOnce:fn=>fn()})};
}
test('disabled execution, absent/mismatched/expired approval never reaches ledger or provider',async()=>{
 for(const edit of [c=>c.executionEnabled=false,c=>c.approval=undefined,c=>c.approval.manifestSha256='d'.repeat(64),c=>c.approval.expiresAt=1000,c=>c.review.manifest.resources.activityVersion=2]) {
