...
test('exact approved manifest submits once after preflight and durable claim; duplicates do not reach provider',async()=>{
 const c=config();const r=await submitReviewedSandbox(c);assert.deepEqual(c.calls,['claim','begin','provider','submitted']);assert.equal(r.approvalGranted,false);assert.ok(!JSON.stringify(r).includes('secret'));
...
test('approval expiry during preflight and tampered session binding stop before claim',async()=>{
 const c=config();let clock=1000;c.now=()=>clock;c.preflight=async()=>{clock=2001;return {manifestSha256:c.review.manifestSha256,submitOnce:fn=>fn()};};await assert.rejects(submitReviewedSandbox(c),/APPROVAL_REQUIRED/);assert.deepEqual(c.calls,[]);
 const d=config();d.preflight=async()=>({manifestSha256:'f'.repeat(64),submitOnce:fn=>fn()});await assert.rejects(submitReviewedSandbox(d),/PREFLIGHT_REQUIRED/);assert.deepEqual(d.calls,[]);
});
test('transport cannot retry provider call or fabricate submission without invoking it',async()=>{
 const c=config();c.preflight=async()=>({manifestSha256:c.review.manifestSha256,submitOnce:async fn=>{await fn();return fn();}});
 ...
 const d=config();d.preflight=async()=>({manifestSha256:d.review.manifestSha256,submitOnce:async()=>({id:'fabricated-id'})});
...
test('deferred callbacks cannot execute after transport failure; expiry is checked inside provider boundary',async()=>{
 ...
 c.preflight=async()=>({manifestSha256:c.review.manifestSha256,submitOnce:async fn=>{deferred=fn;return {id:'fabricated-id'};}});
 ...
 d.preflight=async()=>({manifestSha256:d.review.manifestSha256,submitOnce:fn=>{clock=2001;return fn();}});
 await assert.rejects(submitReviewedSandbox(d),/OUTCOME_UNKNOWN/);assert.equal(d.calls.filter(x=>x==='provider').length,0);
});
