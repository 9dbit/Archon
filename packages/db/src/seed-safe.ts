...
export async function seedCasaNusaOnce(databaseUrl: string) {
  ...
  const [existing] = await db.select({ id: projects.id }).from(projects).where(eq(projects.name, 'Casa Nusa Restaurant')).limit(1);
  if (existing) return { projectId: existing.id, seeded: false };
