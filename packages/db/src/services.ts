...
export async function approveChangeSet(db:ArchonDatabase,input:{changeSetId:string;reviewer:string;note?:string}){
 return db.transaction(async tx=>{
  ...
  const snapshot={schemaVersion:1,projectId:changeSet.projectId,baseVersionId:changeSet.baseVersionId,committedChangeSet:{id:changeSet.id,intentSummary:changeSet.intentSummary,operations:changeSet.operations,affectedDomains:changeSet.affectedDomains,requestedLocks:changeSet.requestedLocks}};
  ...
  await tx.insert(auditEvents).values([{projectId:changeSet.projectId,entityType:'ChangeSet',entityId:changeSet.id,eventType:'APPROVED_AND_COMMITTED',actor:input.reviewer,payload:{versionId:version.id,versionNumber:version.versionNumber}},{projectId:changeSet.projectId,entityType:'ProjectVersion',entityId:version.id,eventType:'CREATED',actor:input.reviewer,payload:{changeSetId:changeSet.id}}]); return version;
 });
}
