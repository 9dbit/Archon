  ...
  return db.transaction(async tx=>{
    ...
    for(const operation of input.operations){
      ...
      if(operation.type==='MOVE'){
        const values=['deltaXmm','deltaYmm','deltaZmm'].map(key=>operation.payload[key]).filter(value=>value!==undefined);
        if(!values.length||values.some(value=>!finiteNumber(value)))throw new Error('INVALID_MOVE_PAYLOAD');
      }
    }
    ...
    await tx.insert(auditEvents).values({projectId:input.projectId,entityType:'ChangeSet',entityId:changeSet.id,eventType:'PROPOSED',actor:input.createdBy,payload:{operations:input.operations,intentSummary:input.intentSummary}});
    return changeSet;
  });
}

function materializeCanonicalObjects(objects:any[],operations:CanonicalOperation[],nextVersionNumber:number){
  const operationMap=new Map<string,CanonicalOperation[]>();
  for(const operation of operations){const list=operationMap.get(operation.targetId)??[];list.push(operation);operationMap.set(operation.targetId,list)}
  return objects.map(item=>{
    const itemOperations=operationMap.get(item.archonId)??[];
    if(!itemOperations.length)return {...item};
    let parameters={...(item.parameters??{})};
    for(const operation of itemOperations){
      if(operation.type==='MOVE'){
        const position=tuple3(parameters.positionMm);
        parameters={...parameters,positionMm:[position[0]+(finiteNumber(operation.payload.deltaXmm)?operation.payload.deltaXmm as number:0),position[1]+(finiteNumber(operation.payload.deltaYmm)?operation.payload.deltaYmm as number:0),position[2]+(finiteNumber(operation.payload.deltaZmm)?operation.payload.deltaZmm as number:0)]};
      } else if(operation.type==='UPDATE') parameters={...parameters,...operation.payload};
    }
    return {...item,parameters,revision:item.revision+1,provenance:{...(item.provenance??{}),source:'ARCHON_CHANGESET',version:String(nextVersionNumber),authority:'BUILDING'}};
  });
}
...
export async function approveChangeSet(db:ArchonDatabase,input:{changeSetId:string;reviewer:string;note?:string}){
 return db.transaction(async tx=>{
  ...
  await tx.insert(approvals).values({changeSetId:changeSet.id,decision:'APPROVED',reviewer:input.reviewer,note:input.note});
  await tx.update(changeSets).set({state:'COMMITTING',updatedAt:new Date()}).where(eq(changeSets.id,changeSet.id));
  ...
  const currentObjects=await tx.select().from(canonicalObjects).where(eq(canonicalObjects.projectId,changeSet.projectId));
  const materializedObjects=materializeCanonicalObjects(currentObjects,(changeSet.operations??[]) as CanonicalOperation[],nextVersionNumber);
  ...
  const canonicalSnapshot=materializedObjects.map(({id,projectId,...item})=>item);
  const snapshot={schemaVersion:2,projectId:changeSet.projectId,baseVersionId:changeSet.baseVersionId,canonicalObjects:canonicalSnapshot,committedChangeSet:{id:changeSet.id,intentSummary:changeSet.intentSummary,operations:changeSet.operations,affectedDomains:changeSet.affectedDomains,requestedLocks:changeSet.requestedLocks}};
