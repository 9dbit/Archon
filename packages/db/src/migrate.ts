...
export async function migrate(databaseUrl: string) {
  ...
  try {
    ...
    const [existing] = await sql<{ id: string }[]>`
      SELECT id FROM archon_migrations WHERE id = ${MIGRATION_ID} LIMIT 1
    `;

    if (existing) {
      console.log(`[ARCHON DB] ${MIGRATION_ID} already applied`);
      return;
    }

    const migrationPath = fileURLToPath(
      new URL('../drizzle/0000_archon_foundation.sql', import.meta.url)
    );
    ...
    await sql.begin(async (tx) => {
      ...
      await tx`
        INSERT INTO archon_migrations (id) VALUES (${MIGRATION_ID})
