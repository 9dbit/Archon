...
import {activateSandboxLedgerTransaction,sandboxLedgerDDL,sandboxReceiptDDL} from './sandbox-activation.mjs';
...
test('real activation is idempotent, keeps unrelated data, and rejects malformed existing schema',{skip:!process.env.ARCHON_LEDGER_TEST_DATABASE_URL},async()=>{
 ...
 try {
  ...
  for(let i=0;i<2;i++){const result=await activateSandboxLedgerTransaction(transaction);assert.equal(result.ledgerReady,true);assert.equal(result.receiptStoreReady,true);}
